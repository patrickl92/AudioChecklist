describe("SoundChecklistItem", function()
    local soundChecklistItem

    local function createItem()
        return soundChecklistItem:new("Challenge Key")
    end

    setup(function()
        soundChecklistItem = require "audiochecklist.soundchecklistitem"
    end)

    teardown(function()
        soundChecklistItem = nil
    end)
    ...
    it("should initialize a new checklist", function()
        ...
        assert.are.equal(soundChecklistItem.stateNotStarted, checklistItem:getState())
    end)

    it("should thrown an error if the challenge key is invalid", function()
        assert.has_error(function() soundChecklistItem:new(nil) end, "challengeKey must be a string")
        assert.has_error(function() soundChecklistItem:new(0) end, "challengeKey must be a string")
    end)

    it("should set its state", function()
        local checklistItem = createItem()

        checklistItem:setState(soundChecklistItem.stateInProgress)
        assert.are.equal(soundChecklistItem.stateInProgress, checklistItem:getState())
