describe("SOPExecutor",function()
    ...
    local function createChallengeVoice()
        ...
        stub.new(voice, "onActivated")
        stub.new(voice, "onDeactivated")
    ...
    local function createResponseVoice()
        ...
        stub.new(voice, "onActivated")
        stub.new(voice, "onDeactivated")
        stub.new(voice, "playChallengeSound", function() error("Should not call playChallengeSound on challenge voice") end)
    ...
    it("should activate and deactivate the SOP", function()
        ...
        assert.stub(challengeVoice.onActivated).was.called(1)
        assert.stub(responseVoice.onActivated).was.called(1)

        assert.stub(sop.onDeactivated).was_not_called()
        assert.stub(challengeVoice.onDeactivated).was_not_called()
        assert.stub(responseVoice.onDeactivated).was_not_called()

        sopExecutor.setActiveSOP(nil)

        assert.stub(sop.reset).was.called(2)
        assert.stub(sop.onDeactivated).was.called(1)
        assert.stub(challengeVoice.onDeactivated).was.called(1)
        assert.stub(responseVoice.onDeactivated).was.called(1)
    end)
    ...
    it("should switch between SOPs", function()
        ...
        assert.stub(challengeVoice2.onActivated).was.called(1)
        assert.stub(responseVoice2.onActivated).was.called(1)

        sopExecutor.setActiveSOP(nil)

        assert.stub(sop2.reset).was.called(2)
        assert.stub(sop2.onDeactivated).was.called(1)
        assert.stub(challengeVoice2.onDeactivated).was.called(1)
        assert.stub(responseVoice2.onDeactivated).was.called(1)
    end)
